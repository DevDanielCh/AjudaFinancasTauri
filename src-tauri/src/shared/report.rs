...
pub fn sync_generated(conn: &Connection, now: NaiveDate) -> Result<(), String> {
    ...
    while m <= now {
        ...
        let count: i64 = conn
            ...
            .map_err(db_err)?;
        if count > 0 {
            crate::domain::generate_fixed_bills(conn, m)?;
...
fn build(conn: &rusqlite::Connection, month: &str) -> Result<DashboardData, String> {
    ...
    crate::domain::generate_fixed_bills(conn, ref_month)?;
    ...
    let income = month_income(conn, ref_month, ref_month.checked_add_months(Months::new(1)).unwrap())?;
    let expenses = month_expenses(conn, ref_month)?;
    let prev_income = month_income(conn, prev, ref_month)?;
    let prev_expenses = month_expenses(conn, prev)?;
    ...
    let expenses_by_pm = expenses_by_pm(conn, ref_month)?;
    ...
    let (balance, prev_balance) = if settings.primeiro_mes.is_some() {
        (
            account_balance_at(conn, next)?,
            account_balance_at(conn, ref_month)?,
        )
    } else {
        (
...
fn build_chart(conn: &rusqlite::Connection, month: &str) -> Result<ChartData, String> {
    ...
    let next = ref_month.checked_add_months(Months::new(1)).unwrap();
    Ok(ChartData {
        monthly: monthly_series(conn, ref_month)?,
        expenses_by_cat: expenses_by_category(conn, ref_month, next)?,
        expenses_by_pm: expenses_by_pm(conn, ref_month)?,
    })
}
